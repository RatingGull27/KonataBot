import discord, lavalink, logging, re, math, config
from discord.ext import commands

regex = re.compile('[0-9]+')

class Music:
    """ Music commands that can soothe your ears. """
    def __init__(self, bot):
        self.bot = bot

        if not hasattr(bot, 'lavalink'):
            lavalink.Client(bot=bot, password=config.lavalink['pwd'], loop=self.bot.loop, log_level=logging.DEBUG)
    ...
    async def track_hook(self, event):
        if isinstance(event, lavalink.Events.TrackStartEvent):
            ...
            if c:
                ...
                if c:
                    embed = discord.Embed(colour=0x885EAD, description=f"```ini\n[ Now Playing ]\n{event.track.title}```")
            ...
            if c:
                c = self.bot.get_channel(c)
                if c:
                    await c.send('<:KonataOk:438856307580338176> **|** Queue ended, wanna queue more songs?')
